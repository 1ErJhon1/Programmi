...
int menu(){
	int num;
	cout<<"-------INTERFACCIA MENU'-------"<<endl<<"1) carica dipendenti"<<endl<<"2) ordina i dipendenti in ordine alfabetico"<<endl<<"3) aumenta stipendio dipendenti livello 4"<<endl<<"4) salva i dati aggiornati nel file di testo"<<endl<<"5) aggiungi dipendente"<<endl<<"0) fine programma"<<endl<<"inserisci l'operazione da svolgere: ";
	cin>>num;
	cout<<endl;
	return num;
}
...
int aggpers(fstream &file,int dim){
	...
	cin>>agg.lvl;
	cout<<endl;
	file<<endl<<agg.cognome<<endl<<agg.stip<<endl<<agg.lvl;
	file.close();
	dim++;
	return dim;
}