...
void vismat(int mat[][cmax],int r,int c){
	...
	for(i=0;i<r;i++){
		for(j=0;j<c;j++){
			cout<<"Il numero posizionato in: "<<i+1<<" "<<j+1<<"= "<<mat[i][j]<<endl;}}
}
...
void maxr(int mat[][cmax],int r, int c){
	...
	for(i=0;i<r;i++){
		...
		if(mat[i][j]>max)
		max=mat[i][j];}
		cout<<"il numero maggiore della riga: "<<i+1<<" = "<<max<<endl;}
}
...
void mintab(int mat[][cmax],int r, int c){
	...
	for(i=0;i<r;i++){
		...
		if(mat[i][j]<min)
		min=mat[i][j];}}
		cout<<"Il numero minimo della tabella e': "<<min<<endl;
}
...
void medcol(int mat[][cmax],int r, int c){
	...
	som=0;
		...
		som=mat[i][j]+som;}
		med=som/r;
		cout<<"la media della colonna: "<<j+1<<" e'= "<<med<<endl;}
}
